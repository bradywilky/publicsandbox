...
def _get_bounds():
    ...
    for sun in sun_json['sun_list']:
        if datetime.fromtimestamp(sun['date']).date() == datetime.today().date():
            return datetime.fromtimestamp(sun['rise']).strftime('%H:%M'), datetime.fromtimestamp(sun['set']).strftime('%H:%M')
    return '06:00', '19:00'
...
def set_background_color():
    ...
    if day_begin <= datetime.now().strftime('%H:%M') < day_end:
...
def get_color(layout, for_matplotlib=False):
    ...
    if day_begin <= datetime.now().strftime('%H:%M') < day_end: # Daytime color
        ...
        color_dict['widget_alt3'] = "rgb(12, 12, 20)" # time display font color
        ...
        color_dict['widget_alt1'] = "rgb(12, 12, 20)"
        color_dict['widget_alt2'] = "rgb(12, 12, 20)" 
        color_dict['widget_alt3'] = "rgb(255, 255, 255)" # time display font color
